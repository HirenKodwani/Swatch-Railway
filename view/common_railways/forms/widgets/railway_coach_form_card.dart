...
import '../../scorecard_forms/common_coach_cleaning_scorecard.dart';


class RailwayCoachFormCard extends StatefulWidget {
  final CoachForm form;
  final VoidCallback? onView;
  final VoidCallback? onPdf;
  final VoidCallback? onScore;

  const RailwayCoachFormCard({
    super.key,
    required this.form,
    this.onView,
    this.onPdf,
  ...
  @override
  State<RailwayCoachFormCard> createState() => _RailwayCoachFormCardState();
}

class _RailwayCoachFormCardState extends State<RailwayCoachFormCard> {
          ...
          Text.rich(
            TextSpan(
              children: [
                ...
                if(widget.form.submittedByDepot != null && widget.form.submittedByDepot!.isNotEmpty)
                const TextSpan(
                    text: ' •  Depot: ',
                    style: TextStyle(fontWeight: FontWeight.w500)),
  ...
  bool _canShowActionButtons() {
    ...
    return user?.role == 'Railway Supervisor' || user?.role == 'Contractor Supervisor';
  }
  ...
  Widget _buildMainActionButton() {
    ...
    if (status == 'APPROVED_BY_RAILWAY' || status == 'SCORING_IN_PROGRESS') {
      if (canPerformActions) {
        onPressed = () async {
          final result = await Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  CommonCoachCleaningScorecard(form: widget.form),
            ),
          );
      ...
      if (canPerformActions) {
        onPressed = _isApproving ? null : () => _approveForm(widget.form, context);
      } else {
        onPressed = null;
      }
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: isLoading
                    ...
                    try {
                      final result = await ApiService.rejectCoachForm(
  ...
  void _approveForm(CoachForm form, BuildContext context) async {
    setState(() => _isApproving = true);

    try {
      await ApiService.approveManpower(formId: form.uid);
