...
// ─── PEST / RODENT CONTROL ──────────────────────────────────────────────────
app.post('/api/station-pest-control/record', verifyToken, async (req, res) => {
  try {
    const { stationId, stationName, area, zone, pestType, severity, treatmentMethod, chemicalsUsed, notes, evidencePhotos } = req.body;
    if (!stationId || !pestType || !severity) {
      return res.status(400).json({ error: 'stationId, pestType, severity are required' });
    }
    ...
    const data = {
      ...
      recordedBy: req.user?.uid || req.user?.userId || 'unknown',
      recordedByName: req.user?.fullName || req.user?.name || 'Unknown',
      status: 'PENDING_REVIEW',
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      treatedAt: null, reviewedBy: null, reviewNotes: '', reviewedAt: null
    };
    await ref.set(data);
    res.status(201).json({ success: true, message: 'Pest control record created', uid: ref.id, data });
  } catch (error) {
    console.error('(PestControl) Error recording:', error);
    res.status(500).json({ error: 'Failed to record pest control activity', details: error.message });
  }
});

app.get('/api/station-pest-control/list/:stationId', verifyToken, async (req, res) => {
  try {
    const { stationId } = req.params;
    const { status, pestType, fromDate, toDate } = req.query;
    let query = db.collection('pestControlRecords').where('stationId', '==', stationId);
    if (status) query = query.where('status', '==', status);
    if (pestType) query = query.where('pestType', '==', pestType);
    const snapshot = await query.orderBy('createdAt', 'desc').get();
    let records = [];
    snapshot.forEach(doc => records.push(doc.data()));
    if (fromDate) records = records.filter(r => r.createdAt?.toDate() >= new Date(fromDate));
    if (toDate) records = records.filter(r => r.createdAt?.toDate() <= new Date(toDate));
    res.status(200).json({ success: true, count: records.length, data: records });
  } catch (error) {
    console.error('(PestControl) Error listing:', error);
    res.status(500).json({ error: 'Failed to list pest control records', details: error.message });
  }
});

app.get('/api/station-pest-control/all', verifyToken, async (req, res) => {
  try {
    const { status, fromDate, toDate, zone, division } = req.query;
    let query = db.collection('pestControlRecords');
    if (status) query = query.where('status', '==', status);
    const snapshot = await query.orderBy('createdAt', 'desc').get();
    let records = [];
    snapshot.forEach(doc => records.push(doc.data()));
    if (fromDate) records = records.filter(r => r.createdAt?.toDate() >= new Date(fromDate));
    if (toDate) records = records.filter(r => r.createdAt?.toDate() <= new Date(toDate));
    if (zone) records = records.filter(r => r.zone === zone);
    if (division) records = records.filter(r => r.division === division);
    res.status(200).json({ success: true, count: records.length, data: records });
  } catch (error) {
    console.error('(PestControl) Error listing all:', error);
    res.status(500).json({ error: 'Failed to list pest control records', details: error.message });
  }
});

app.put('/api/station-pest-control/:uid/review', verifyToken, async (req, res) => {
  try {
    const { uid } = req.params;
    const { status, reviewNotes, treatmentDate } = req.body;
    if (!['APPROVED', 'REJECTED', 'FOLLOW_UP'].includes(status)) {
      return res.status(400).json({ error: 'Status must be APPROVED, REJECTED, or FOLLOW_UP' });
    }
    const updateData = {
      status, reviewNotes: reviewNotes || '',
      reviewedBy: req.user?.uid || req.user?.userId || 'unknown',
      reviewedAt: admin.firestore.FieldValue.serverTimestamp()
    };
    if (treatmentDate) updateData.treatedAt = treatmentDate;
    await db.collection('pestControlRecords').doc(uid).update(updateData);
    res.status(200).json({ success: true, message: `Pest control record ${status.toLowerCase()}` });
  } catch (error) {
    console.error('(PestControl) Error reviewing:', error);
    res.status(500).json({ error: 'Failed to review pest control record', details: error.message });
  }
});

app.get('/api/station-pest-control/report', verifyToken, async (req, res) => {
  try {
    const { fromDate, toDate, zone, division, stationId } = req.query;
    let query = db.collection('pestControlRecords');
    const snapshot = await query.get();
    let records = [];
    snapshot.forEach(doc => records.push(doc.data()));
    ...
    res.status(200).json({ success: true, summary, byPestType, data: records });
  } catch (error) {
    console.error('(PestControl) Error report:', error);
    res.status(500).json({ error: 'Failed to generate pest control report', details: error.message });
  }
});

// ─── MACHINE / MATERIAL DEPLOYMENT ──────────────────────────────────────────
app.post('/api/station-machines/deploy', verifyToken, async (req, res) => {
  try {
    const { stationId, stationName, machineName, machineType, quantity, deployedTo, deployedToName, deploymentDate, expectedReturnDate, condition, notes } = req.body;
    if (!stationId || !machineName || !quantity) {
      return res.status(400).json({ error: 'stationId, machineName, quantity are required' });
    }
    const ref = db.collection('machineDeployments').doc();
    const data = {
      uid: ref.id, stationId, stationName: stationName || '',
      machineName, machineType: machineType || 'General',
      quantity: parseInt(quantity) || 1,
      deployedTo: deployedTo || '', deployedToName: deployedToName || '',
      deployedBy: req.user?.uid || req.user?.userId || 'unknown',
      deployedByName: req.user?.fullName || req.user?.name || 'Unknown',
      deploymentDate: deploymentDate || new Date().toISOString(),
      expectedReturnDate: expectedReturnDate || '',
      condition: condition || 'GOOD',
      status: 'DEPLOYED', notes: notes || '',
      returnedAt: null, returnedCondition: '', returnNotes: '',
      createdAt: admin.firestore.FieldValue.serverTimestamp()
    };
    await ref.set(data);
    res.status(201).json({ success: true, message: 'Machine deployed', uid: ref.id, data });
  } catch (error) {
    console.error('(MachineDeploy) Error:', error);
    res.status(500).json({ error: 'Failed to deploy machine', details: error.message });
  }
});

app.get('/api/station-machines/list', verifyToken, async (req, res) => {
  try {
    const { stationId, status, machineType } = req.query;
    let query = db.collection('machineDeployments');
    if (stationId) query = query.where('stationId', '==', stationId);
    if (status) query = query.where('status', '==', status);
    if (machineType) query = query.where('machineType', '==', machineType);
    const snapshot = await query.orderBy('createdAt', 'desc').get();
    let items = [];
    snapshot.forEach(doc => items.push(doc.data()));
    res.status(200).json({ success: true, count: items.length, data: items });
  } catch (error) {
    console.error('(MachineList) Error:', error);
    res.status(500).json({ error: 'Failed to list machines', details: error.message });
  }
});

app.put('/api/station-machines/:uid/return', verifyToken, async (req, res) => {
  try {
    const { uid } = req.params;
    const { returnedCondition, returnNotes } = req.body;
    await db.collection('machineDeployments').doc(uid).update({
      status: 'RETURNED',
      returnedAt: admin.firestore.FieldValue.serverTimestamp(),
      returnedCondition: returnedCondition || '',
      returnNotes: returnNotes || '',
      returnedBy: req.user?.uid || req.user?.userId || 'unknown'
    });
    res.status(200).json({ success: true, message: 'Machine returned' });
  } catch (error) {
    console.error('(MachineReturn) Error:', error);
    res.status(500).json({ error: 'Failed to return machine', details: error.message });
  }
});

app.put('/api/station-machines/:uid/maintenance', verifyToken, async (req, res) => {
  try {
    const { uid } = req.params;
    const { maintenanceNotes, maintenanceDate } = req.body;
    await db.collection('machineDeployments').doc(uid).update({
      status: 'MAINTENANCE',
      maintenanceNotes: maintenanceNotes || '',
      maintenanceDate: maintenanceDate || new Date().toISOString(),
      lastMaintenanceBy: req.user?.uid || req.user?.userId || 'unknown'
    });
    res.status(200).json({ success: true, message: 'Machine marked for maintenance' });
  } catch (error) {
    console.error('(MachineMaintenance) Error:', error);
    res.status(500).json({ error: 'Failed to update machine', details: error.message });
  }
});

app.get('/api/station-machines/report', verifyToken, async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, summary, byType, data: items });
  } catch (error) {
    console.error('(MachineReport) Error:', error);
    res.status(500).json({ error: 'Failed to generate machine report', details: error.message });
  }
});

// ─── GARBAGE DISPOSAL RECORD ────────────────────────────────────────────────
app.get('/api/station-garbage/records', verifyToken, async (req, res) => {
  try {
    const { stationId, fromDate, toDate } = req.query;
    let query = db.collection('garbageDisposalRecords');
    if (stationId) query = query.where('stationId', '==', stationId);
    const snapshot = await query.orderBy('createdAt', 'desc').get();
    let records = [];
    snapshot.forEach(doc => records.push(doc.data()));
    ...
    console.error('(GarbageRecords) Error:', error);
    res.status(500).json({ error: 'Failed to fetch garbage records', details: error.message });
  }
});

app.post('/api/station-garbage/record', verifyToken, async (req, res) => {
  try {
    const { stationId, stationName, area, wasteType, quantityKg, disposalMethod, disposalAgency, vehicleNo, notes, evidencePhotos } = req.body;
    if (!stationId || !wasteType || !quantityKg) {
      return res.status(400).json({ error: 'stationId, wasteType, quantityKg are required' });
    }
    ...
    const data = {
      ...
      disposalMethod: disposalMethod || '', disposalAgency: disposalAgency || '',
      vehicleNo: vehicleNo || '', notes: notes || '',
    ...
    await ref.set(data);
    res.status(201).json({ success: true, message: 'Garbage disposal recorded', uid: ref.id, data });
  } catch (error) {
    console.error('(GarbageRecord) Error:', error);
    res.status(500).json({ error: 'Failed to record garbage disposal', details: error.message });
  }
});

app.get('/api/station-garbage/report', verifyToken, async (req, res) => {
  try {
    const { fromDate, toDate, stationId } = req.query;
    let query = db.collection('garbageDisposalRecords');
    if (stationId) query = query.where('stationId', '==', stationId);
    const snapshot = await query.get();
    let records = [];
    ...
    res.status(200).json({ success: true, totalRecords: records.length, totalKg, byWasteType, data: records });
  } catch (error) {
    console.error('(GarbageReport) Error:', error);
    res.status(500).json({ error: 'Failed to generate garbage report', details: error.message });
  }
});
